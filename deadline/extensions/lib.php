...
class extensions_plugin extends deadline_plugin {
    ...
    const EXTENSION_INDIVIDUAL = 0;
    const EXTENSION_GROUP      = 1;
    const EXTENSION_GLOBAL     = 2;

    // Define the constants for extensions being enabled.
    const EXTENSION_NO_SEL     = -1;
    const EXTENSION_DISABLED   =  0;
    const EXTENSION_ENABLED    =  1;

    // Define all the statuses here.
    const EXTENSION_STATUS_NONE      = 0;
    const EXTENSION_STATUS_PENDING   = 1;
    const EXTENSION_STATUS_APPROVED  = 2;
    const EXTENSION_STATUS_DENIED    = 3;
    const EXTENSION_STATUS_WITHDRAWN = 4;
    const EXTENSION_STATUS_REVOKED   = 5;
    const EXTENSION_STATUS_MOREINFO  = 6;
    const EXTENSION_STATUS_DELETED   = 7;
    ...
    /**
     * Build a form to present to the user when adding or editing an activity to
     * enable or disable extensions.
     *
     * @param object $mform MoodleForm item generated by Moodle
     * @param int $cm_id Course Module ID if known (will be present on edit)
     */
    ...
    /**
     * Check to see if an activity has a submission form the specified student
     *
     * @param int $cm_id Course Module ID to check
     * @param int $user_id User ID to check for submissions.
     */
    ...
    /**
     * Get an array of timelimit selection options.
     *
    ...
    public static function get_all_extension_status() {
        $status = array();
        $status[self::EXTENSION_STATUS_NONE]      = get_string('status_none',      self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_PENDING]   = get_string('status_pending',   self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_APPROVED]  = get_string('status_approved',  self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_DENIED]    = get_string('status_denied',    self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_WITHDRAWN] = get_string('status_withdrawn', self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_REVOKED]   = get_string('status_revoked',   self::EXTENSIONS_LANG);
        $status[self::EXTENSION_STATUS_MOREINFO]  = get_string('status_moreinfo',  self::EXTENSIONS_LANG);
    ...
    public static function get_extension_enable_items() {
        return array(
                self::EXTENSION_NO_SEL   => '&nbsp;',
                self::EXTENSION_DISABLED => get_string('no'),
                self::EXTENSION_ENABLED  => get_string('yes')
        );
    }
    ...
    /**
     * Get a list of all extension requests for a specific Course Module ID and User ID.
     *
     * @param int $cm_id Course Module ID to check
     * @param int $user_id User ID to check for.
     * @throws coding_exception
     * @return Mixed Array of extension objects if found, otherwise false if none found.
     */
    ...
    /**
     * Given a Course Module ID find the module type.
     *
    ...
    /**
     * Get activity ID for specific extension ID.
     *
     * @param int $ext_id Extension ID.
     */
    ...
    /**
     * Check to see if a specific user is an extension approver for a specific extension
     *
     * @param int $ext Extension ID to check.
     * @param int $user User ID to check
     * @param int $cmid Course Module ID to check.
     * @return boolean
     */
    ...
    public static function is_enabled() {
        if (get_config('deadline_extensions', 'enabled') == extensions_plugin::EXTENSION_ENABLED) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Get a specific grouoing ID for an activity based on Course Module ID.
     *
     * @param int $cm_id Course Module ID to check
     * @return mixed Object containing grouping ID if exists, false if not.
     */
    ...
    public static function get_pending_count_for_user($user_id = null, $course_id = null) {
        ...
        return self::get_count_all_extensions_by_staffid($user_id, self::EXTENSION_STATUS_PENDING, $course_id);
    }
        ...
        if (get_config(self::EXTENSIONS_MOD_NAME, 'show_pending_count')) {
            ...
            if (is_null($course) || $course->id == '0') {
                ...
                foreach ($courses as $course) {
                    $count += self::get_count_all_extensions_by_staffid($user->id, self::EXTENSION_STATUS_PENDING, $course->id);
                }
    ...
    /**
     * Check to see if an activity allows group submissions
     *
     * @param int $cmid Course Module ID to check for group submissions.
     * @return int Grouping ID if one is set.
     */
    ...
    /**
     * Check to see if extension is a timelimit extension rather than a date
     * extension
     *
     * @param int $ext_id Extension ID to check
     * @return boolean True if it timelimit extension, otherwise false.
     */
    ...
    public static function save_quick_approve($form_data) {
        ...
        foreach ($form_data->extension_requests as $ext_id => $status) {
            ...
            $item = new stdClass;
            $item->id = $ext_id;
            $item->status = self::EXTENSION_STATUS_APPROVED;
    ...
    /**
     * Get the group extensions for an activity based on course module id.
     *
     * @param int $cm_id
     * @return object Object containing the global extensions for the specified activity.
     */
    ...
    /**
     * Get the global extensions for an activity based on course module ID.
     *
     * @param int $cm_id Course Module ID.
     * @return object Object containing the global extensions for the specified activity.
     */
    public function get_activity_global_extensions($cm_id) {
        ...
        $params = array(
                'cm_id' => $cm_id,
                'ext_type' => self::EXTENSION_GLOBAL
        );
    ...
    /**
     * Build the table of extension requests presented to students.
     *
     * @param int $student_id Student ID to use
    ...
    public static function build_activity_table($activities) {
        ...
        foreach ($activities as $activity) {
            ...
            if ($activity->ext_status) { // if the activity has extensions enabled.
                ...
                $pending_count->text    = self::get_extensions_count($activity->id, self::EXTENSION_STATUS_PENDING);
                $approved_count->text   = self::get_extensions_count($activity->id, self::EXTENSION_STATUS_APPROVED);
                $denied_count->text     = self::get_extensions_count($activity->id, self::EXTENSION_STATUS_DENIED);
    ...
    /**
     * Build UI table of global extensions to present to the user.
     *
     * @param int $course Course ID to get activites for.
     * @return html_table Object containging the HTML Table data.
     */
    ...
    /**
     * Get the individual approved extensions for a course module ID and user id.
     *
     * @param int $cm_id Course Module ID.
     * @param int $user_id User ID.
     * @param string $field Field to get data for.
     */
    public function get_individual_approved_extensions($cm_id, $user_id, $field = 'date') {
        ...
        $params = array(
                'cm_id'      => $cm_id,
                'student_id' => $user_id,
                'status'     => self::EXTENSION_STATUS_APPROVED,
                'ext_type'   => self::EXTENSION_INDIVIDUAL
        );
    ...
    /**
     * Get group approved extensions for a course module ID and user id.
     *
     * @param int $cm_id Course Module ID to get data for.
     * @param int $user_id User ID to use to check for
     * @param boolean $global True if check for global extension, galse if check for individual extension.
     * @param string $field Field to check the date for.
     * @return number
     */
    public function get_group_approved_extensions($cm_id = null, $user_id = null, $global = false, $field = 'date') {
        ...
        $params['cm_id']    = $cm_id;
        $params['status']   = self::EXTENSION_STATUS_APPROVED;

        // If this is a global extension then the type of extension will be different.
        if ($global) {
            $params['ext_type'] = self::EXTENSION_GLOBAL;
        } else {
            $params['ext_type'] = self::EXTENSION_GROUP;
        }
    ...
    /**
     * @see deadline_plugin::get_my_open_date()
     */
    ...
    /**
     * Get the specific timelimit for a user based on course module ID and User ID.
     *
     * @see deadline_plugin::get_my_timelimit()
     * @param int $cm_id Course Module ID to look for
     * @param int $user_id User to check for extension for.
     */
    ...
    /**
     * Notify the user using the standard Moodle functionality to advise of a change to
     * the extension.
     *
     * @param object $form_data Form data as submitted to a moodle form.
     */
    public static function notify_user($form_data) {
        ...
        if ($form_data->ext_status_code == self::EXTENSION_STATUS_APPROVED) {
        ...
        if ($form_data->ext_status_code == self::EXTENSION_STATUS_DENIED) {
        ...
        if ($form_data->ext_status_code == self::EXTENSION_STATUS_WITHDRAWN) {
        ...
        if ($form_data->ext_status_code == self::EXTENSION_STATUS_REVOKED) {
        ...
        if ($form_data->ext_status_code == self::EXTENSION_STATUS_MOREINFO) {
